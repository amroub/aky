...
using Diatly.Foundation.Akeneo.Model;
...
namespace Akeneo.Common
{
	public class EndpointResolver
	{
        ...
        private static readonly Type AttributeGroup = typeof(AttributeGroup);
        private static readonly Type ProductModels = typeof(ProductModel);
		...
		public string ForResourceType<TModel>(string parentCode = null) where TModel : ModelBase
		{
            ...
            var isVariant = FamilyVariant.GetTypeInfo().IsAssignableFrom(typeof(TModel));
			return isOption? $"{Endpoints.Attributes}/{parentCode}/options"
				: isVariant? $"{Endpoints.Families}/{parentCode}/variants"
		...
		public string ForResource<TModel>(TModel existing) where TModel : ModelBase
		{
            ...
            var variant = existing as FamilyVariant;
            if (variant != null)
            {
                return $"{baseUri}/{option.Attribute}/variants/{option.Code}";
            }
            ...
            var attributeGroup = existing as AttributeGroup; 
            if (attributeGroup != null)
            {
                return $"{baseUri}/{attributeGroup.Code}";
            }

            var productModel = existing as ProductModel;
            if (productModel != null)
            {
                return $"{baseUri}/{productModel.Code}";
            }
		...
		protected virtual string GetResourceEndpoint(Type modelType)
		{
			return _typeToEndpointCache.GetOrAdd(modelType, type =>
			{
                ...
                if (ProductModels.GetTypeInfo().IsAssignableFrom(type))
                {
                    return Endpoints.ProductModels;
                }
                ...
                if (AttributeGroup.GetTypeInfo().IsAssignableFrom(type))
                {
                    return Endpoints.AttributeGroup;
                }
